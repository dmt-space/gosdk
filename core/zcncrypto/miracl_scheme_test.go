...
func TestMiraclGenerateKeys(t *testing.T) {
...
func TestMiraclSignAndVerifyWithNewKeys(t *testing.T) {
	sigScheme := NewSignatureScheme("bls0chain")
	switch sigScheme.(type) {
	case SignatureScheme:
		// pass
	...
	ok, err := sigScheme.Verify(sign, msg)
	require.Nil(t, err)
	require.True(t, ok)

}

func TestMiraclSignAndVerify(t *testing.T) {
	...
	pub.DeserializeHexStr(verifyPublickey)

	// pub, err := hex.DecodeString(verifyPublickey)
	// fmt.Println(string(pub))

	str := HerumiToMiraclPK(pub.GetHexString())
